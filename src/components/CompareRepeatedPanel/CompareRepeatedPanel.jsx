...
export default function CompareRepeatedPanel({ album, progress }) {
  ...
  const handleToggleTradePick = useCallback((code) => {
    setTradePickCodes((prev) =>
      prev.includes(code) ? prev.filter((c) => c !== code) : [...prev, code].sort((a, b) => a.localeCompare(b)),
    );
  }, []);
      ...
      <div className={styles.root}>
        ...
        <div className={styles.bodyRow}>
          <p className={styles.hint}>
            Pegá la lista FIGURITAPP (u otro texto compatible) y comparala con lo que te falta en el álbum.
          </p>
        ...
        >
          <div className={styles.dialog} role="dialog" aria-modal="true" aria-labelledby={titleId}>
            <div className={styles.dialogHeader}>
              ...
              <button type="button" className={styles.closeBtn} onClick={handleClose} aria-label="Cerrar ventana de repes">
                Cerrar
              </button>
            </div>
            ...
            <div className={styles.dialogBody}>
              <p className={styles.intro}>
                Pegá la lista de repes que te pasaron. Comparamos contra tu álbum (solo lectura: no marcamos nada como
                conseguido).
              </p>
              ...
              {result ? (
                <>
                  {summarySentence ? <p className={styles.summary}>{summarySentence}</p> : null}
                  ...
                  <section className={styles.group} aria-labelledby={`${titleId}-g-useful`}>
                    ...
                    {result.useful.length === 0 ? (
                      <p className={styles.emptyGroup}>No hay coincidencias útiles para tu álbum.</p>
                    ) : (
                    ...
                    <section className={styles.group} aria-labelledby={`${titleId}-g-unknown`}>
                      ...
                      <ul className={styles.resultListPlain}>
                        {allUnknown.map((item, index) => (
                          ...
                          >
                            <span className={styles.unknownCode}>{item.value ?? item.code ?? "—"}</span>
